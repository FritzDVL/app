...
export function FeaturedCommunities({ featuredCommunities }: FeaturedCommunitiesProps) {
  return (
    <div className="w-full overflow-hidden rounded-3xl border border-slate-300/60 bg-white backdrop-blur-sm">
      <div className="border-b border-slate-200/80 bg-gradient-to-r from-slate-100/90 to-white px-4 py-4 sm:px-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="truncate text-lg font-semibold text-slate-900">Featured Communities</h3>
          <Link
            href="/communities"
        ...
        <div className="w-full space-y-3">
          {featuredCommunities.map(community => (
            <Link
              ...
              className="group block w-full cursor-pointer rounded-xl border border-slate-200/60 bg-white p-3 transition-all hover:-translate-y-0.5 hover:border-brand-300/60 hover:shadow-sm"
            >
              <div className="flex w-full min-w-0 items-center gap-3">
                ...
                <div className="shrink-0">
                  <ArrowRight className="h-4 w-4 shrink-0 text-slate-400 opacity-0 transition-all group-hover:translate-x-1 group-hover:opacity-100" />
                </div>
              </div>
            </Link>
          ))}
